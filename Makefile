...
$(OUT)/mwcceppc.elf: $(OUT)/generated.o $(OUT)/compat.o $(OUT)/genstr.o $(OUT)/main.o | $(OUT)
	$(CC) $(CFLAGS) -static -no-pie -o $@ $^

$(OUT):
	mkdir -p $(OUT)

$(OUT)/main.o: main.c compat.h | $(OUT)
	$(CC) $(CFLAGS) -static -no-pie -c -o $@ $<

$(OUT)/compat.o: compat.c compat.h | $(OUT)
	$(CC) $(CFLAGS) -static -no-pie -c -o $@ $<

$(OUT)/genstr.o: $(OUT)/genstr.c | $(OUT)
	$(CC) $(CFLAGS) -static -no-pie -c -o $@ $<

$(OUT)/generated.o $(OUT)/genstr.c: $(OUT)/pe2elf mwcceppc.exe mwcceppc_syms.txt | $(OUT)
