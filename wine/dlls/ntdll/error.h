...
static const struct { ULONG start, end, offset; } ntdll_error_table[43] =
{
...
{
    int low = 0, high = ARRAY_SIZE(ntdll_error_table) - 1;

    while (low <= high)
    {
        int pos = (low + high) / 2;
        if (status < ntdll_error_table[pos].start) high = pos - 1;
        else if (status > ntdll_error_table[pos].end) low = pos + 1;
        else return error_map[ntdll_error_table[pos].offset + (status - ntdll_error_table[pos].start)];
    }
